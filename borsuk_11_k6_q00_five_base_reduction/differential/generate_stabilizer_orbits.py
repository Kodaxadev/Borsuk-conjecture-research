...
def generate_rows() -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    report: dict[str, object] = {
        "schema": "borsuk-q00-fifth-base-differential-v1",
        "n": N,
        "diameter": K,
        "q00_base": list(BASE),
        ...
        "status": "differential verifier only; all governed children UNKNOWN",
...
def main() -> int:
    ...
    if digest != EXPECTED_CSV_SHA256:
        raise SystemExit(f"differential CSV hash mismatch: {digest}")
    report["differential_csv_sha256"] = digest
