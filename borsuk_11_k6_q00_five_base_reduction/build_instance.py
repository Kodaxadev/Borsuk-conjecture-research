...
def get_case(case_id: str) -> dict[str, object]:
    ...
    for row in rows:
        if row["id"] == case_id:
            return row
    raise SystemExit(f"unknown q00 five-base case id: {case_id}")


def build_graph(case: dict[str, object]) -> tuple[list[int], list[tuple[int, int]]]:
    base = Q00_BASE + (int(case["D"]),)
    ...
    if len(vertices) != int(case["vertices"]) or len(edges) != int(case["exact_distance_edges"]):
        raise SystemExit("regenerated graph statistics disagree with five-base case record")
...
def fixed_clique_for(vertices: list[int], node_limit: int = CLIQUE_SEARCH_NODE_LIMIT) -> list[int]:
    ...
    if len(clique) > COLORS:
        raise SystemExit("symmetry-breaking clique exceeds color count")
    return clique


def allowed_colors_for(vertices: list[int], fixed_clique: list[int]) -> dict[int, tuple[int, ...]]:
...
def mapping_document(
    vertices: list[int], fixed_clique: list[int], domains: dict[int, tuple[int, ...]], clauses: int
) -> dict[str, object]:
...
def instance_metadata(
    ...
    return {
        "schema": "borsuk-q00-five-base-list-color-instance-v1",
        "case_id": case["id"],
        ...
        "edges": len(edges),
        "incompatible_pairs": int(case["incompatible_pairs"]),
        "colors": COLORS,
        "fixed_clique": fixed_clique,
...
def emit_cnf(
    ...
    variables, clauses = formula_counts(vertices, edges, domains)
    stream.write("c Borsuk n=11 k=6 q00 five-base compact 12-list-coloring encoding\n")
    ...
    for left, right in edges:
        ...
        while common:
            ...
            common -= bit
            stream.write(
                f"-{pair_to_variable[(left, color)]} -{pair_to_variable[(right, color)]} 0\n"
            )
...
def main() -> int:
    ...
    mapping = mapping_document(vertices, fixed_clique, domains, int(metadata["clauses"]))
