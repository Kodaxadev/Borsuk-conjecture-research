...
const stored = path.join(__dirname, 'canonical_12.csv');
if (fs.existsSync(stored) && fs.readFileSync(stored, 'ascii') !== csv) throw new Error('stored CSV differs from independent generation');
console.log(JSON.stringify({status:'PASS', stabilizer_order:EXPECTED_STABILIZER, candidates:432, orbits:rows.length, child_range:[Math.min(...rows.map(r=>r.vertices)),Math.max(...rows.map(r=>r.vertices))], csv_sha256:digest}, null, 2));
