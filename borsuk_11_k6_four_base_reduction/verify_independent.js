...
const EXPECTED_CSV_SHA256 = 'e60f5f81120e42c8c7eae2da299105e802266f6e92629cd6ac6ee24bcef9db19';
...
for (const [family, b] of Object.entries(B_CASES)) {
  ...
  while (unseen.size > 0) {
    ...
    rows.push([
      ...
      actual.C,
      actual.vertices,
      actual.exact_distance_edges,
      actual.incompatible_pairs,
      actual.vertex_sha256,
    ].join(','));
...
const csv = rows.join('\n') + '\n';
const csvHash = crypto.createHash('sha256').update(csv, 'ascii').digest('hex');
if (totalPointwise !== 149 || totalSetwise !== 101 || minTrim !== 436 || maxTrim !== 612) {
  throw new Error(`totals ${totalPointwise} ${totalSetwise} ${minTrim} ${maxTrim}`);
}
if (csvHash !== EXPECTED_CSV_SHA256) throw new Error(`case-list hash ${csvHash}`);
console.log(`PASS pointwise=${totalPointwise} setwise=${totalSetwise} trim_range=${minTrim}-${maxTrim} csv_sha256=${csvHash}`);
